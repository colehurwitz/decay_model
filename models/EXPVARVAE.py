...
class EXPVARVAE(nn.Module):
    def __init__(self, training_set, args, abs_, optimize_both_exp=True, capacity1=50, capacity2=25, dropout1=.75, dropout2=.5, batchnorm=True):
        ...
        #self.abvars = torch.tensor(self.abvar.repeat(num_spikes).view(num_spikes, 3), requires_grad=True)
        
        if(args.cuda):
            if(optimize_both_exp):
                self.exps = torch.tensor(abs_, requires_grad=True, device="cuda")
                self.vars = torch.tensor(var.repeat(self.num_spikes).view(self.num_spikes, 1), requires_grad=True, device="cuda")
            else:   
            ...
            if(optimize_both_exp):
                ...
                self.vars = torch.tensor(var.repeat(self.num_spikes).view(self.num_spikes, 1), requires_grad=True)
            else:   
    ...
    def forward(self, amps, ch_locs, spike_ids):
        x_mu, x_var, y_mu, y_var, z_mu, z_var = self.encode(amps)
        if(self.training):
        ...
        recon_amps = self.decode(sampled_n_loc, ch_locs, spike_ids)
...
def loss_function(recon_amps, t_amps, x_mu, x_var, y_mu, y_var, z_mu, z_var, MSEs, xklds, yklds, zklds, epochs, sigmas, device):
...
#     sigma_repeated = sigmas.repeat(recon_amps.shape[1]).view(batch_size, recon_amps.shape[1])
    G_dist = torch.distributions.Normal(recon_amps, sigmas)
    GLL = torch.sum(G_dist.log_prob(t_amps[:,:,0])*t_amps[:,:,1])/batch_size

    m_qx = x_mu
    m_px = torch.tensor(0).float().to(device)
    var_qx = x_var
    var_px = torch.tensor(50**2).float().to(device) 
    KLD_x = kl_divergence_normal(m_qx, var_qx, m_px, var_px)/batch_size
    xklds.append(KLD_x)

    m_qy = y_mu
    m_py = torch.tensor(0).float().to(device)
    var_qy = y_var
    var_py = torch.tensor(50**2).float().to(device) 
    KLD_y = kl_divergence_normal(m_qy, var_qy, m_py, var_py)/batch_size
    yklds.append(KLD_y)


    m_qz = z_mu
    m_pz = torch.tensor(0).float().to(device)
    var_qz = z_var
    var_pz = torch.tensor(50**2).float().to(device) 
    KLD_z = kl_divergence_normal(m_qz, var_qz, m_pz, var_pz)/batch_size
    zklds.append(KLD_z)
    
    return -GLL + KLD_x + KLD_y + KLD_z
...
def train(model, device, args, optimizer, train_loader, epoch, train_losses, distance_diffs, a_list, b_list, MSEs, xklds, yklds, zklds):
    model.train()
    train_loss = 0
    for batch_idx, (t_amps, t_ch_locs, center_loc, spike_ids, _)  in enumerate(train_loader):
        t_amps = t_amps.to(device)
        t_ch_locs = t_ch_locs.to(device)
        spike_ids = spike_ids.to(device)
        optimizer.zero_grad()
        recon_amps, x_mu, x_var, y_mu, y_var, z_mu, z_var = model(t_amps, t_ch_locs, spike_ids)
        sigmas = torch.index_select(model.vars, 0, spike_ids)
        loss = loss_function(recon_amps, t_amps, x_mu, x_var, y_mu, y_var, z_mu, z_var, MSEs, xklds, yklds, zklds, epoch, sigmas, device)
        loss.backward()
